...
nextPrimeOffset aM limit start =
  do l <- marksLimit aM limit
     let loop i = if i > l
                  then return (l + 1)
                  else do v <- readArray aM i
                          if v == 0 then loop (i + 2) else return i;
     loop (start + 1 + (start .&. 1))
...
--   do pV <- newArray (0, n - 1) 0
--      cV <- newArray (0, n - 1) 0
--      mapM_ (\(i, p, c) -> writeArray pV i p >> writeArray cV i c) (zip3 [n-1, n-2 .. 0] primes cursors)
--      return (pV, cV)
...
-- sumMarks :: Marks s -> Int64 -> ST s Int64
-- sumMarks aM nL =
--   do l <- marksLimit aM nL
--      foldM (\s i -> readArray aM i >>= return . (+ s) . fromIntegral) 0 [0 .. l]
